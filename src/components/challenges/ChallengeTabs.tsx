    ...
    <div className='font-semibold text-[18px] flex'>
      {ChallengeOptions.map(({ label }) => (
        <h1
          ...
          className={`cursor-pointer flex-1/3 h-[61px] flex items-center justify-center box-border border-b-[5px] ${level === label ? 'border-[#007AFF] text-[#007AFF]' : 'border-[#1a1a1a] text-[#414141]'}`}
        >
          {label}
        </h1>
      ))}
